...
const BeneficiaryChart: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const uniqueContractors = Array.from(new Set(filteredBudgets.map(project => project.name)));
    ...
    // Update chart data
    setState({
      series: [30,20], // Assuming 65, 40 are other data points
      labels: [`  Interventions (${30})`,` projects (${20})` ],
    });
  ...
  const uniqueDates = Array.from(new Set(budgets.map(budget => budget.budget_yr.toString())));
  return (
    ...
    <select
          ...
          >
            <option value="">Select An MDA</option>
              ...
              <option> PBC</option>.
              <option> KDBS</option>
              <option> Pension Bureau</option>
              <option> MOA</option>
              <option> KADCHMA</option>
